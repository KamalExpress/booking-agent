...
- **Tenant Inbox Feature:** Implement the Tenant Inbox UI. This needs to support autogenerated system notifications, manually sent messages from the SaaS Admin, and a reply mechanism (while disabling replies for autogenerated system alerts).
...
- **Fix Worker Stalling (Proxy State Leak):** *Deferred until after system functionality is tested.* When assignments complete or expire, the `lease_service.py` functions (`expire_stale_leases`, `complete_lease`, `cancel_active_leases`) fail to reset the `PortalAccount` and `Proxy` statuses back to `"READY"`. This causes healthy resources to become permanently stuck in the `"LEASED"` state, preventing the scheduler from assigning new leases and resulting in workers sleeping indefinitely. This needs to be patched before production rollout.
