...
| `TypeError: ScoringPolicy.score_proxy() takes 1 positional argument but 2 were given` | Worker `/api/v1/worker/assignments/next` returned HTTP 500 when scheduling leases. | `scheduler_service.py` was updated to pass `due_assignment.provider` to `score_proxy`, but `scoring_policy.py` on production had an older method signature expecting only `(proxy)`. | Added optional `task_provider: str = "GVC"` with `*args, **kwargs` to `score_proxy` and `score_account` on both branches. | Fixed via commit `8f3aefe` (`staging`) and `730a610` (`prod`). |
