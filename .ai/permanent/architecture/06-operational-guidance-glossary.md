...
| `WEEKEND_SCHEDULE_FILTER` | Staff reported workers checking Saturdays and Sundays, which are off. | Date generator looped `current_date += timedelta(days=1)` across all calendar days without checking `weekday`. | Filtered all date ranges with `weekday < 5` (Monday to Friday only). Mapped National Visa Type D (2) to Thursday & Friday, and Seasonal Type D (26), Prime Time (6), and Premium Lounge (5) across all weekdays (Mon-Fri). | Fixed via commit `be6554c` (`staging`) and `47c93ff` (`prod`). |
...
| `REGIONAL_DOMAIN_FAILOVER` | Target portal domain routing shifts from `pk-gr-services` to `bd-gr-services`. | During peak slot open windows, extreme traffic on `pk-gr` triggers origin overload and strict Imperva WAF challenges. Neighboring regional endpoints share the backend DB/auth but experience low traffic and baseline WAF scrutiny. | Configure `BOOKING_PORTAL_URL=https://bd-gr-services.gvcworld.eu` on worker nodes during slot open days. Documented in `.ai/lessons/regional-domain-routing-evasion.md`. | Ready for dynamic routing in Control Plane. |
## Production & Staging Operational Event Dictionary (August 31, 2026)

| Event / Error Signature | What Happened | Why did it happen? | How to fix it | Auto-Recovery |
| --- | --- | --- | --- | --- |
| `LEASE_RESULT` | Worker submitted check or booking cycle result (`status: COMPLETED` or `FAILED`, `reason: NO_SLOTS`). | Worker node completed its assigned lease and reported status telemetry to `/api/v1/worker/logs`. | Dashboard automatically formats event with meaningful context (`[LEASE COMPLETED] Check cycle finished successfully (reason)`). | Logged and displayed automatically in Live Activity widget. |
| `HALLUCINATED_TOOL_ALIAS` | AI emitted invented tool name (e.g. `get_worker_healthy`, `worker_status`, `check_proxies`). | Small language models (BitNet 2.7B) construct intuitive names based on prompt words rather than exact registry strings. | Resolved via `TOOL_ALIAS_MAP` in `TravelOSMCPClient.call_tool` and closest tool fallback mapping, eliminating permission rejection errors. | Yes. Automatically translated to canonical tool at runtime. |
| `TYPED_EVIDENCE_FIREWALL_INTERCEPT` | Model attempted to answer an operational inquiry on hop 0 or invoked an irrelevant tool (e.g. `get_proxy_health` for worker failure). | Model attempted ungrounded text generation or selected a misaligned tool during investigation. | Grounding Firewall intercepts the output, suppresses ungrounded text, and forces invocation of the required typed evidence tool (`get_worker_logs`). | Yes. Intercepted and routed automatically by Copilot orchestrator. |
| `DUAL_BOUNDARY_TEMPORAL_WINDOW` | User asked about events in a specific time window (e.g. "why did the worker fail 7-8 minutes ago?"). | Worker events require precise time-bounded database queries to avoid surfacing stale or irrelevant historical errors. | `_parse_temporal_boundaries` extracts `(since_minutes, until_minutes)` and filters `EventLog`/`WorkerLog` within `[now - since, now - until]`. Returns `[NO_ERRORS_RECORDED]` if window has no events. | Handled automatically in FastMCP tools. |
| `QUANTIFIED_FLEET_STATUS` | User asked binary health questions ("all workers healthy?", "any worker unhealthy?"). | Small models hedge with vague answers ("There are 8 workers, some are online..."). | Grounding validator inspects `[WORKER FLEET] X/Y online` and structures firm binary verdict: `"No. 2 of 8 workers are online; 6 are offline. None are currently reporting an error."` | Deterministically formatted by orchestrator. |
