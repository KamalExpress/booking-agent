...
- **Assignment:** A specific configuration (Visa Center, Date Range, Appointment Type) created by a Tenant that needs to be continuously monitored for slots.
- **Lease:** A temporary lock acquired by a Worker Node on an Assignment. Prevents two workers from monitoring the exact same assignment simultaneously.
