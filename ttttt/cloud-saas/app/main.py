...
class MCPAuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        if mcp_key:
            ...
            if not token:
                token = request.query_params.get("api_key") or request.query_params.get("token") or request.headers.get("x-watchdog-key") or ""
            if token != mcp_key:
                return JSONResponse(
                    {"error": "Unauthorized: Invalid or missing MCP API key. Provide 'Authorization: Bearer <key>', 'X-Watchdog-Key', or '?api_key=<key>'"},
                    status_code=401
                )
...
try:
    ...
    logger.info("Mounted FastMCP at /mcp with MCPAuthMiddleware")
except Exception as e:
    logger.warning(f"Failed to mount FastMCP: {e}")
